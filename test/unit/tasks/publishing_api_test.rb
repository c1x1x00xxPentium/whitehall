...
class PublishingApiRake < ActiveSupport::TestCase
  ...
  describe "republish" do
    ...
    describe "#all_*" do
      let(:tasks) do
        [
          { name: "all_organisations", model: "Organisation" },
          { name: "all_take_part_pages", model: "TakePartPage" },
          { name: "all_people", model: "Person" },
          { name: "all_roles", model: "Role" },
          { name: "all_role_appointments", model: "RoleAppointment" },
        ]
      end

      test "republishes all records of a model" do
        tasks.each do |task|
          create(task[:model].underscore) # rubocop:disable Rails/SaveBang
      ...
      describe "#document" do
        let(:task) { Rake::Task["publishing_api:republish:document"] }
